  ...
  return {
    ...
    me: {
      ...
      savedAcademies: {
        ...
        list: () => request<{ items: Record<string, unknown>[] }>('GET', '/me/saved-academies'),
      },
      getMyReviews: () =>
        request<{
          items: Array<{
      ...
      referral: {
        get: () => request<{ code: string; points: number }>('GET', '/me/referral'),
        claim: (code: string) =>
          request<{ success: boolean; points_earned: number }>('POST', '/me/referral/claim', {
            code,
          }),
        history: () =>
          request<{ points: number; history: Record<string, unknown>[] }>(
            'GET',
            '/me/referral/history'
          ),
      },
    ...
    batches: {
      ...
      getTrialAvailability: (params: { id: string }) =>
        request<{
          ...
          accepting: boolean;
          times: string[];
        }>('GET', `/batches/${params.id}/trial-availability`),
    ...
    sessions: {
      upcoming: () => request<UpcomingSessionsResponse>('GET', '/me/sessions/upcoming'),
    },
    enrollments: {
      get: (id: string) =>
    ...
    bookings: {
      ...
      reschedule: (params: { id: string; new_trial_at: string }) =>
        request<{ booking: Record<string, unknown> }>('POST', `/bookings/${params.id}/reschedule`, {
    ...
    coaching: {
      ...
      checkIn: (params: { id: string; otp_code: string }) =>
        request<{ booking: CoachBooking }>(
          'POST',
          `/studio/coaching/requests/${params.id}/checkin`,
          { otp_code: params.otp_code }
        ),
      checkOut: (params: {
        id: string;
        reason: 'completed' | 'student_left' | 'coach_left' | 'no_show' | 'academy_no_fulfill';
      }) =>
        request<{ booking: CoachBooking }>(
          'POST',
          `/studio/coaching/requests/${params.id}/checkout`,
          { reason: params.reason }
        ),
    },
    trials: {
    ...
    attendance: {
      checkin: (token: string) =>
        request<{ present: boolean; batch_title: string }>('POST', '/attendance/checkin', {
          token,
        }),
    },
    reviews: {
    ...
    workshops: {
      ...
      get: (id: string) => request<{ workshop: any }>('GET', `/workshops/${id}`),
      register: (id: string) =>
        request<{ registration_id: string; requires_payment: boolean; amount_paise: number }>(
          'POST',
      ...
      cancelRegistration: (
        ...
        request<{
          ...
          refund_initiated: boolean;
          refund_amount_paise: number;
        }>('POST', `/workshops/registrations/${registrationId}/cancel`, {
    ...
    notes: {
      ...
      remove: (id: string) => request<{ deleted: boolean }>('DELETE', `/notes/${id}`),
    },
    push: {
    ...
    notifications: {
      ...
      markRead: (id: string) => request<{ ok: boolean }>('POST', `/notifications/${id}/read`),
    ...
    resources: {
      ...
      listForBatchAcademy: (batchId: string) =>
        request<ResourcesListResponse>(
          'GET',
          `/studio/batches/${encodeURIComponent(batchId)}/resources`
        ),
      ...
      remove: (id: string) => request<{ deleted: boolean }>('DELETE', `/studio/resources/${id}`),
    },
    studio: {
      dashboard: () => request<DashboardData>('GET', '/studio/dashboard'),
      inbox: {
        list: (query?: Record<string, unknown>) => {
          const qs = query
            ? `?${new URLSearchParams(stripUndefined(query) as any).toString()}`
            : '';
      ...
      schedule: {
        get: (query?: Record<string, unknown>) => {
          const qs = query
            ? `?${new URLSearchParams(stripUndefined(query) as any).toString()}`
            : '';
          return request<{ days: { date: string; items: ScheduleItem[] }[] }>(
            'GET',
            `/studio/schedule${qs}`
          );
        },
      ...
      sessions: {
        cancel: (body: { batch_id: string; session_date: string }) =>
          request<{ cancelled: boolean; affected_count: number; already_cancelled?: boolean }>(
            'POST',
            '/studio/sessions/cancel',
            body
          ),
        ...
        markAbsent: (
          batchId: string,
          date: string,
          body: { reason?: string; recording_url?: string }
        ) =>
          request<{ marked_absent: number }>(
            'POST',
            `/studio/batches/${batchId}/sessions/${date}/absent`,
            body
          ),
      },
      slots: {
        ...
        delete: (id: string) => request<{}>('DELETE', `/studio/slots/${id}`),
        reschedule: (id: string, body: { slot_time: string }) =>
          request<{}>('PUT', `/studio/slots/${id}`, body),
      },
      programs: {
        list: () => request<{ items: Record<string, unknown>[] }>('GET', '/studio/programs'),
        get: (id: string) =>
        ...
        delete: (id: string) => request<{}>('DELETE', `/studio/programs/${id}`),
      },
      batches: {
        list: () => request<{ items: Batch[] }>('GET', '/studio/batches'),
        get: (id: string) => request<{ batch: Batch }>('GET', `/studio/batches/${id}`),
        ...
        delete: (id: string) => request<{}>('DELETE', `/studio/batches/${id}`),
        // Batch discontinuation lifecycle (active → closing → ended).
        discontinue: (id: string) =>
          request<{ status: string; cancelled_trials: number; notified: number }>(
            'POST',
            `/studio/batches/${id}/discontinue`
          ),
        ...
        refundDiscontinuation: (id: string, enrollmentId: string) =>
          ...
          }>('POST', `/studio/batches/${id}/discontinuation/refund`, {
        ...
        students: (batchId: string) =>
          ...
          }>('GET', `/studio/batches/${batchId}/students`),
        ...
        attendance: {
          ...
          sessionAttendance: (batchId: string, date?: string) => {
            ...
            return request<{
              ...
              total: number;
              roster: { user_id: string; name: string; checked_in: boolean; marked_at?: string }[];
            }>('GET', `/studio/batches/${batchId}/session/attendance${qs}`);
          },
          // issue today's QR check-in token for an in-studio session.
          checkinToken: (batchId: string) =>
            request<{ token: string; session_date: string }>(
              'POST',
              `/studio/batches/${batchId}/session/checkin-token`
            ),
        },
      ...
      students: {
        list: (query?: Record<string, unknown>) => {
          const qs = query
            ? `?${new URLSearchParams(stripUndefined(query) as any).toString()}`
            : '';
          return request<{ items: StudentListItem[]; next_cursor: string | null }>(
            'GET',
            `/studio/students${qs}`
          );
        },
        get: (id: string) =>
          request<{
            ...
            enrollments: unknown[];
            trial_history: unknown[];
            attendance_history: unknown[];
          }>('GET', `/studio/students/${id}`),
      },
      reviews: {
        list: (query?: {
          filter?: 'all' | 'needs_reply' | 'replied' | '5' | 'lte3';
          cursor?: string;
        }) => {
          const qs = query
            ? `?${new URLSearchParams(stripUndefined(query) as any).toString()}`
            : '';
          return request<{ items: Review[]; summary?: ReviewsSummary; next_cursor: string | null }>(
            'GET',
            `/studio/reviews${qs}`
          );
        },
      ...
      settings: {
        get: () => request<{ settings: Settings }>('GET', '/studio/settings'),
        update: (body: Partial<Settings>) =>
          request<{ settings: Settings }>('PUT', '/studio/settings', body),
      },
      academy: {
        get: () => request<Record<string, unknown>>('GET', '/studio/academy'),
        update: (body: Record<string, unknown>) =>
          request<Record<string, unknown>>('PUT', '/studio/academy', body),
      },
      workshops: {
        list: () => request<{ items: Workshop[] }>('GET', '/studio/workshops'),
        ...
        delete: (id: string) => request<{}>('DELETE', `/studio/workshops/${id}`),
      ...
      coaches: {
        ...
        delete: (id: string) => request<{}>('DELETE', `/studio/coaches/${id}`),
      },
    },
    rooms: {
