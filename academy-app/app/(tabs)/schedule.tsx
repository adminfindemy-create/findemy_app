...
import { useTheme, sansFor, Button, IconChevL, IconChevR, IconX } from '@findemy/ui';
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <View style={{ backgroundColor: s.bg, borderRadius: 999, paddingHorizontal: 9, paddingVertical: 4 }}>
      <Text
        style={{
...
export default function ScheduleScreen() {
  ...
  const confirmCancel = async () => {
    ...
    try {
      await cancelSession.mutateAsync({ batch_id: cancelPick.batch_id, session_date: cancelPick.session_date });
      setCancelOpen(false);
      setCancelPick(null);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.head}>
          ...
          <Pressable
            style={[styles.batchesBtn, { backgroundColor: theme.color.ivory, borderColor: theme.color.hairline }]}
            onPress={() => router.push('/programs' as any)}
          >
            <Text style={{ fontFamily: sansFor(600), fontSize: 12, color: theme.color.ink }}>Programs</Text>
          </Pressable>
        </View>
        ...
        <View style={styles.weekNav}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            {!isCurrentWeek ? (
              ...
              >
                <Text style={{ fontFamily: sansFor(700), fontSize: 11, color: theme.color.persimmonDeep }}>Today</Text>
              </Pressable>
            ) : null}
          </View>
          <Pressable
          ...
          {weekDays.map((day) => {
            ...
            return (
              ...
              >
                ...
                <Text style={{ fontFamily: theme.font.serif, fontSize: 18, lineHeight: 22, marginTop: 2, color: selected ? theme.color.ivory : theme.color.ink }}>
                  {format(day, 'd')}
                </Text>
                <View style={[styles.dot, { backgroundColor: count > 0 ? theme.color.persimmon : 'transparent' }]} />
              </Pressable>
            );
          })}
        </View>
            ...
            <>
              <View style={styles.secHead}>
                <Text style={{ fontFamily: sansFor(800), fontSize: 20, letterSpacing: -0.4, color: theme.color.ink }}>
                  ...
                  {focusedIsToday ? <Text style={{ fontFamily: theme.font.serifItalic, color: theme.color.persimmon }}> · today</Text> : null}
                </Text>
              ...
              {dayItems.length === 0 ? (
                ...
                <View style={{ gap: 10 }}>
                  {dayItems.map((item) => {
                    ...
                    const catColor = cat === 'arts' ? theme.color.persimmon : (CATEGORY_COLORS[cat] ?? theme.color.mist);
                    const cancelled = item.status === 'cancelled';
                    const online = item.mode === 'online';
                    return (
                      <Pressable
                        key={`${item.batch_id}:${item.session_date}`}
                        style={[styles.sch, { backgroundColor: theme.color.ivory, borderColor: theme.color.hairline, opacity: cancelled ? 0.72 : 1 }, theme.shadow.sm]}
                        onPress={() =>
                      ...
                      >
                        <View style={styles.rail}>
                          ...
                          <Text style={{ fontFamily: sansFor(600), fontSize: 11, color: theme.color.whisper, marginTop: 1 }}>
                            {format(start, 'a')}
                          </Text>
                        </View>
                        ...
                        <View style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {cancelled ? (
                            <Text style={{ fontFamily: sansFor(500), fontSize: 12, color: theme.color.rose, marginTop: 3 }} numberOfLines={2}>
                              Cancelled · +1 make-good session (count preserved)
                            </Text>
                          ) : (
                            <Text style={{ fontFamily: sansFor(500), fontSize: 12, color: theme.color.mist, marginTop: 3 }} numberOfLines={1}>
                              {[item.coach_name, `${item.enrolled_count ?? 0}/${item.capacity ?? 0} students`].filter(Boolean).join(' · ')}
                              {online ? (
                                <Text style={{ fontFamily: sansFor(700), color: theme.color.jade }}>{'  Online'}</Text>
                              ) : null}
                            </Text>
                          )}
                        </View>
      ...
      <Modal visible={cancelOpen} transparent animationType="slide" onRequestClose={() => setCancelOpen(false)}>
        <Pressable style={styles.sheetOverlay} onPress={() => setCancelOpen(false)}>
          <Pressable style={[styles.sheet, { backgroundColor: theme.color.paper }]} onPress={() => {}}>
            <View style={[styles.grab, { backgroundColor: theme.color.hairline }]} />
            <View style={styles.sheetHead}>
              <Text style={{ fontFamily: theme.font.serif, fontSize: 24, color: theme.color.ink }}>Cancel a class</Text>
            ...
            <View style={{ gap: 8, marginBottom: 12 }}>
              {cancellable.map((item) => {
                const sel = cancelPick && cancelPick.batch_id === item.batch_id && cancelPick.session_date === item.session_date;
                const start = new Date(item.start_time ?? '');
                const timeLabel = isNaN(start.getTime()) ? '' : ` · ${format(start, 'h:mm a')}`;
                return (
                  <Pressable
                    key={`${item.batch_id}:${item.session_date}`}
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontFamily: sansFor(700), fontSize: 14, color: theme.color.ink }}>
                        {item.batch_title}{timeLabel}
                      </Text>
                      <Text style={{ fontFamily: sansFor(500), fontSize: 12, color: theme.color.mist, marginTop: 2 }}>
                        {[item.coach_name, `${item.enrolled_count ?? 0} students`].filter(Boolean).join(' · ')}
                      </Text>
                    </View>
            ...
            <View style={[styles.policy, { backgroundColor: theme.color.jadeSoft }]}>
              <Text style={{ fontFamily: theme.font.sans, fontSize: 12.5, lineHeight: 18, color: theme.color.jade }}>
                <Text style={{ fontFamily: sansFor(800) }}>Not billed.</Text> A cancelled class isn't counted as a paid session — <Text style={{ fontFamily: sansFor(800) }}>1 make-good session is added</Text> so each student still gets their full plan (e.g. 8 classes stays 8, not a bonus class).
              </Text>
            </View>
