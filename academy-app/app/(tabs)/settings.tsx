...
export default function SettingsScreen() {
  ...
  const setNotif = (
    group: 'new_trial' | 'classes' | 'reviews_activity' | 'quiet_hours',
    key: string,
    value: boolean
  ) => {
  ...
  const Row = ({
    ...
    >
      <View style={{ flex: 1, paddingRight: 12 }}>
        <Text style={{ fontFamily: sansFor(600), fontSize: 14, color: theme.color.ink }}>
          {label}
        </Text>
        {sub ? (
          <Text
    ...
    <Screen header={<ScreenHeader title="Settings" showBack />} bottomTab="settings" scroll>
      <View style={{ paddingTop: 8, paddingBottom: 16 }}>
        ...
        <Card>
          <Row
            label="Push"
            value={notifications?.new_trial?.push ?? true}
            onChange={(enabled) => setNotif('new_trial', 'push', enabled)}
          />
          <Row
            label="WhatsApp"
            sub={settings?.contact?.whatsapp}
            value={notifications?.new_trial?.whatsapp ?? false}
            onChange={(enabled) => setNotif('new_trial', 'whatsapp', enabled)}
          />
          <Row
            label="Email"
            sub={settings?.contact?.email}
            value={notifications?.new_trial?.email ?? false}
            onChange={(enabled) => setNotif('new_trial', 'email', enabled)}
            last
          />
        </Card>
        ...
        <Card>
          <Row
            label="Reminder 30 min before"
            value={notifications?.classes?.reminder_30min ?? true}
            onChange={(enabled) => setNotif('classes', 'reminder_30min', enabled)}
          />
          <Row
            label="Attendance reminder"
            value={notifications?.classes?.attendance_reminder ?? true}
            onChange={(enabled) => setNotif('classes', 'attendance_reminder', enabled)}
            last
          />
        </Card>
        ...
        <Card>
          <Row
            label="New review"
            value={notifications?.reviews_activity?.new_review ?? true}
            onChange={(enabled) => setNotif('reviews_activity', 'new_review', enabled)}
          />
          <Row
            label="Leaderboard updates"
            value={notifications?.reviews_activity?.leaderboard ?? false}
            onChange={(enabled) => setNotif('reviews_activity', 'leaderboard', enabled)}
          />
          <Row
            label="Reels tagging"
            value={notifications?.reviews_activity?.reels ?? false}
            onChange={(enabled) => setNotif('reviews_activity', 'reels', enabled)}
          />
          <Row
            label="Quiet hours"
            sub={
