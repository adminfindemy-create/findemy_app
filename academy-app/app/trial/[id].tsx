...
export default function TrialDetailScreen() {
  ...
  const payValue = [feePaise != null ? formatRupees(feePaise) : null, trial.payment_method || 'Paid']
    .filter(Boolean)
    .join(' · ');
  ...
  return (
      ...
      footer={
        attended ? undefined : (
          <Button
            variant="dark"
            block
            onPress={() => router.push({ pathname: '/attendance-otp', params: { trialId: id, studentName: student.name } })}
          >
            Mark attendance →
          </Button>
        )
    ...
    >
      <View style={styles.container}>
        ...
        <Text style={{ fontFamily: theme.font.serif, fontSize: 30, color: theme.color.ink, marginTop: 4 }}>
          {trial.batch_title}
        </Text>
        <Text style={{ fontFamily: sansFor(600), fontSize: 13.5, color: theme.color.mist, marginTop: 3, marginBottom: 18 }}>
          {[whenLabel, online ? 'Findemy video room' : trial.distance_km != null ? `${trial.distance_km} km away` : null]
            .filter(Boolean)
            .join(' · ')}
        </Text>
        ...
        {/* Summary */}
        <Summary>
          <SummaryRow icon={<IconUser size={16} color={iconColor} />} label="Student" value={student.name} />
          <SummaryRow icon={<IconCal size={16} color={iconColor} />} label="Coach" value={trial.coach_name || '—'} />
          <SummaryRow
          ...
          />
          <SummaryRow
            icon={<Text style={{ color: iconColor, fontFamily: sansFor(800), fontSize: 13 }}>₹</Text>}
            label="Payment"
            value={payValue}
            last
          />
        ...
        <View style={[styles.notice, { backgroundColor: theme.color.jadeSoft }]}>
          ...
          <Text style={{ fontFamily: sansFor(600), fontSize: 12.5, color: theme.color.jade, flex: 1, lineHeight: 17 }}>
            {online
        ...
        {online && !attended ? (
          <Button
            ...
            style={{ marginTop: 14 }}
            onPress={() => trial.batch_id && router.push(`/batches/${trial.batch_id}/live-class` as never)}
          >
            Start live class
          </Button>
        ) : null}
        ...
        {student.phone ? (
          <View style={styles.contactRow}>
            <View style={{ flex: 1 }}>
              <Button variant="ghost" size="sm" block onPress={() => Linking.openURL(`tel:${student.phone}`)}>
                Call
              </Button>
            </View>
            <View style={{ flex: 1 }}>
              <Button variant="ghost" size="sm" block onPress={() => Linking.openURL(`https://wa.me/${student.phone.replace(/\D/g, '')}`)}>
                WhatsApp
              </Button>
            </View>
          </View>
        ...
        {trial.note ? (
          <View style={[styles.noteCard, { backgroundColor: theme.color.persimmonSoft, borderColor: theme.color.persimmon }]}>
            <Text style={{ fontFamily: sansFor(700), fontSize: 11, color: theme.color.persimmonDeep, letterSpacing: 1.4, marginBottom: 6, textTransform: 'uppercase' }}>
        ...
        {attended ? (
          <View style={[styles.notice, { backgroundColor: theme.color.jadeSoft, marginTop: 4 }]}>
            ...
            <Text style={{ fontFamily: sansFor(600), fontSize: 13, color: theme.color.jade }}>
              Marked present{trial.marked_at ? ` · ${new Date(trial.marked_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: 'numeric', minute: '2-digit' })}` : ''}
            </Text>
          </View>
        ) : null}
      </View>
...
const styles = StyleSheet.create({
  ...
  kicker: { fontFamily: sansFor(700), fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', color: '#EC5A2B' },
  notice: { flexDirection: 'row', gap: 10, alignItems: 'center', borderRadius: 16, padding: 13, marginTop: 16 },
