...
export default function EditProgramScreen() {
  ...
  const onSave = async () => {
    ...
    if (media.length < 3) return Alert.alert('Add media', 'Please keep at least 3 photos or videos.');
    try {
  ...
  return (
    <Screen header={<ScreenHeader title="Edit program" showBack />} bottomTab={null} scroll>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 32 }}>
        {isLoading ? (
          <Text style={{ color: theme.color.mist, fontFamily: theme.font.sans, paddingVertical: 20 }}>Loading…</Text>
        ) : isError || !program ? (
          ...
          <>
            ...
            <Text style={{ fontFamily: theme.font.sans, fontSize: 12, color: theme.color.mist, marginTop: 16, lineHeight: 18 }}>
              Category can't be changed after creation. Batches are edited from each batch.
            </Text>

            <Button variant="primary" block style={{ marginTop: 20 }} loading={updateProgram.isPending} onPress={onSave}>
              Save changes
            </Button>
          </>
        )}
