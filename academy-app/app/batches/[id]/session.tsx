...
import { Button, IconCheck, IconChevL, Input, sansFor, useTheme } from '@findemy/ui';
...
export default function SessionRecordScreen() {
  ...
  const { data, isLoading, refetch } = useSessionAttendance({
    batchId: id,
    date,
    enabled: !cancelled,
  });
  ...
  const dateLabel = (() => {
    ...
    return Number.isNaN(parsedDate.getTime())
      ? date
      : parsedDate.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' });
  })();
        ...
        >
          ...
          <View style={styles.heroTop}>
            ...
            <View
              style={[
                styles.badge,
                { backgroundColor: cancelled ? theme.color.roseSoft : theme.color.jadeSoft },
              ]}
            >
              ...
              <Text
                style={[
                  styles.badgeText,
                  { color: cancelled ? theme.color.rose : theme.color.jade },
                ]}
              >
              ...
              <View style={styles.statRow}>
                ...
                >
                  <Text
                    style={[
                      styles.statNum,
                      { fontFamily: theme.font.serif, color: theme.color.jade },
                    ]}
                  >
                    {present}
                  </Text>
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        fontFamily: theme.font.serif,
                        color: absent > 0 ? theme.color.rose : theme.color.mist,
                      },
                    ]}
                  >
                    {absent}
              ...
              {absent > 0 && date ? (
                <View style={{ marginTop: 18, gap: 10 }}>
                  ...
                  <Input
                    ...
                    value={recordingUrl}
                    onChangeText={(text) => {
                      setRecordingUrl(text);
                      setSaved(false);
                    }}
                    keyboardType="default"
                    error={
                      markAbsent.isError
                        ? ((markAbsent.error as any)?.message ?? 'Could not save')
                        : undefined
                    }
                  />
                  <Button
