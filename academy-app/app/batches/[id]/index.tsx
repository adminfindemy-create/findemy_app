...
import { View, Text, Pressable, StyleSheet, Linking, TextInput, Alert, ScrollView } from 'react-native';
...
export default function BatchHubScreen() {
  ...
  if (isError) {
    return (
      <View style={[styles.root, { backgroundColor: theme.color.ink, paddingTop: insets.top + 12 }]}>
  ...
  const togglePause = () => {
    ...
    Alert.alert(
      ...
      next === 'inactive' ? 'Stops new sign-ups. Enrolled students are notified.' : 'Re-opens the batch for new sign-ups.',
      [
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.color.paperWarm }]}>
      ...
      >
        ...
        <View style={[styles.hero, { backgroundColor: theme.color.ink, paddingTop: insets.top + 12 }]}>
          <View style={styles.heroTop}>
            <BackButton />
            <View style={[styles.badge, { backgroundColor: batch.status === 'closing' || batch.status === 'ended' ? theme.color.roseSoft : theme.color.jadeSoft }]}>
              <Text style={[styles.badgeText, { color: batch.status === 'closing' || batch.status === 'ended' ? theme.color.rose : theme.color.jade }]}>
                {batch.status === 'closing' ? 'Closing' : batch.status === 'ended' ? 'Ended' : status === 'active' ? 'Active' : 'Paused'}
              </Text>
            </View>
            {online ? (
          ...
          <Text style={{ fontFamily: theme.font.serif, fontSize: 28, color: theme.color.ivory, lineHeight: 32 }}>
            {batch.title}
          </Text>
          <Text style={{ fontFamily: sansFor(600), fontSize: 12.5, color: 'rgba(255,255,255,0.7)', marginTop: 6 }}>
            {metaBits}
          </Text>
          ...
          <Text style={{ fontFamily: sansFor(500), fontSize: 11.5, color: 'rgba(255,255,255,0.5)', marginTop: 8, lineHeight: 16 }}>
            Trial spots open automatically from free seats — capacity minus enrolled. No setup needed.
          </Text>
        </View>
          ...
          {panel === 'att' ? (
            <View style={{ gap: 14 }}>
              <View style={[styles.qrLaunch, { backgroundColor: theme.color.ink }]}>
                <Text style={styles.qrKicker}>Today{timeLabel ? ` · ${timeLabel}` : ''}{online ? ' · Findemy room' : ''}</Text>
                <View style={styles.qrCountRow}>
                  <Text style={{ fontFamily: theme.font.serif, fontSize: 30, color: theme.color.ivory }}>
                    {session?.present_count ?? 0}
                  </Text>
                  <Text style={{ fontFamily: sansFor(700), fontSize: 15, color: 'rgba(255,255,255,0.85)' }}>
                ...
                <Pressable style={[styles.jadeBtn, { backgroundColor: theme.color.jade }]} onPress={goSession}>
                  {online ? <IconUsers size={17} color={theme.color.ivory} /> : <IconQr size={17} color={theme.color.ivory} />}
              ...
              {sessionRows.length > 0 ? (
                ...
                <Text style={{ fontFamily: theme.font.sans, fontSize: 13, color: theme.color.mist, paddingHorizontal: 2 }}>
            ...
            <View style={{ gap: 6 }}>
              {batch.description ? (
                <>
                  ...
                  <Text style={{ fontFamily: theme.font.sans, fontSize: 13.5, lineHeight: 20, color: theme.color.inkSoft }}>
                    {batch.description}
                  </Text>
                </>
              ) : null}
              ...
              <View style={styles.twoCol}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    style={[styles.input, { backgroundColor: theme.color.ivory, borderColor: theme.color.hairline, color: theme.color.ink }]}
                  />
                </View>
                <View style={{ flex: 1 }}>
              ...
              {mode === 'online' ? (
                <View style={[styles.notice, { backgroundColor: theme.color.jadeSoft }]}>
                  ...
                  <Text style={{ fontFamily: sansFor(600), fontSize: 12, color: theme.color.jade, flex: 1, lineHeight: 17 }}>
                    Online batches run in a Findemy video room — no link to share. Tap "Start live class" at class time.
                  </Text>
                </View>
              ) : null}

              <Pressable
                style={[styles.saveBtn, { backgroundColor: theme.color.persimmon }, updateBatch.isPending && { opacity: 0.6 }]}
                disabled={updateBatch.isPending}
                onPress={saveSchedule}
              >
                <Text style={{ fontFamily: sansFor(700), fontSize: 14, color: theme.color.ivory }}>Save changes</Text>
              </Pressable>

              <Pressable style={[styles.softBtn, { backgroundColor: theme.color.marigoldSoft }]} onPress={togglePause}>
                <Text style={{ fontFamily: sansFor(700), fontSize: 13.5, color: theme.color.marigold }}>
              ...
              {batch.status === 'ended' ? (
                <Text style={{ fontFamily: sansFor(600), fontSize: 12.5, color: theme.color.mist, textAlign: 'center', marginTop: 14 }}>
                  This batch has been discontinued.
                </Text>
              ) : (
                <Pressable style={styles.linkRow} onPress={() => router.push(`/batches/${id}/discontinue` as never)}>
                  <Text style={{ fontFamily: sansFor(600), fontSize: 13, color: theme.color.rose }}>
                    {batch.status === 'closing' ? 'Discontinuation in progress →' : 'Discontinue this batch'}
                  </Text>
                </Pressable>
              )}
            </View>
      ...
      {panel === 'att' ? (
        <View style={[styles.footer, { backgroundColor: theme.color.paperWarm, borderTopColor: theme.color.hairline, paddingBottom: insets.bottom + 10 }]}>
          <Pressable style={[styles.darkBar, { backgroundColor: theme.color.ink }]} onPress={goSession}>
...
const styles = StyleSheet.create({
  ...
  heroTop: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 14 },
  ...
  qrKicker: { fontFamily: sansFor(700), fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', color: 'rgba(255,255,255,0.55)' },
  qrCountRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 6, marginTop: 7, marginBottom: 3 },
  qrMuted: { fontFamily: sansFor(500), fontSize: 12.5, color: 'rgba(255,255,255,0.62)', lineHeight: 18, textAlign: 'center', marginBottom: 14 },
  jadeBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, alignSelf: 'stretch', paddingVertical: 13, borderRadius: 14 },

  rosterRow: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, paddingHorizontal: 14, borderRadius: 16, borderWidth: 1 },
  av: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },

  fgh: { fontFamily: sansFor(700), fontSize: 12, letterSpacing: 1.2, textTransform: 'uppercase', color: '#8A8071', marginTop: 12, marginBottom: 8 },
  dayRow: { flexDirection: 'row', gap: 6 },
  dayToggle: { flex: 1, height: 40, borderRadius: 12, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center' },
  twoCol: { flexDirection: 'row', gap: 8 },
  input: { paddingVertical: 12, paddingHorizontal: 14, borderRadius: 14, borderWidth: 1.5, fontSize: 15, fontFamily: sansFor(400) },
  notice: { flexDirection: 'row', gap: 10, alignItems: 'center', borderRadius: 14, padding: 12, marginTop: 8 },
  ...
  linkRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, marginTop: 14, paddingVertical: 4 },
