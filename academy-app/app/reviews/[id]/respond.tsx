...
export default function ReviewRespondScreen() {
  ...
  const toneChips: { label: string; text: string }[] = [
    { label: 'Thank them', text: `Thank you so much, ${firstName}! We're thrilled you enjoyed the class and can't wait to see you again. 🙏` },
    { label: 'Apologize', text: `Hi ${firstName}, we're really sorry your experience fell short. We'd love to make it right — please reach out so we can help.` },
    { label: 'Explain', text: `Thanks for the feedback, ${firstName}. Here's some context on what happened, and what we're doing to improve…` },
    { label: 'Invite back', text: `Thank you, ${firstName}! We'd love to have you back — your next session is on us. See you soon!` },
  ];
  ...
  const meta = [
    ...
    review.created_at ? new Date(review.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : null,
  ]
    .filter(Boolean)
    .join(' · ');
  ...
  return (
    <Screen
      ...
      scroll
      footer={
        <Button variant="dark" block onPress={onSubmit} disabled={response.length === 0 || response.length > 500} loading={respond.isPending}>
          Post reply
        </Button>
      }
    >
      <View style={styles.container}>
        ...
        <View style={[styles.policy, { backgroundColor: theme.color.paperWarm }]}>
          ...
          {meta ? <Text style={{ fontFamily: sansFor(600), fontSize: 11.5, color: theme.color.whisper, marginTop: 2 }}>{meta}</Text> : null}
          {review.comment ? (
            <Text style={{ fontFamily: theme.font.serifItalic, fontSize: 14, color: theme.color.ink, marginTop: 10, lineHeight: 20 }}>
              “{review.comment}”
            </Text>
          ) : null}
        </View>
        ...
        <TextInput
          ...
          style={[
            styles.textarea,
            { backgroundColor: theme.color.ivory, borderColor: theme.color.hairline, color: theme.color.ink, fontFamily: theme.font.sans },
          ]}
        />
        <Text style={{ textAlign: 'right', color: theme.color.mist, fontFamily: theme.font.sans, fontSize: 12, marginTop: 4 }}>
          {response.length}/500
        </Text>
        ...
        <View style={[styles.notice, { backgroundColor: theme.color.marigoldSoft }]}>
          ...
          <Text style={{ fontFamily: sansFor(600), fontSize: 12, color: theme.color.inkSoft, flex: 1, lineHeight: 17 }}>
...
const styles = StyleSheet.create({
  ...
  textarea: { borderWidth: 1.5, borderRadius: 14, padding: 14, fontSize: 15, minHeight: 130, textAlignVertical: 'top' },
  notice: { flexDirection: 'row', gap: 10, alignItems: 'center', borderRadius: 14, padding: 13, marginTop: 12 },
});
