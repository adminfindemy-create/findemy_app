...
export function Stepper({ step, total, label }: { step: number; total: number; label: string }) {
  ...
  return (
    <View style={{ marginBottom: 18 }}>
      <Text style={[styles.label, { fontFamily: theme.font.sansBold, color: theme.color.mist }]}>{label}</Text>
      <View style={styles.bar}>
        {Array.from({ length: total }).map((_, i) => (
          <View
            key={i}
            style={[styles.dot, { backgroundColor: i < step ? theme.color.persimmon : theme.color.hairline }]}
          />
        ))}
      </View>
    </View>
