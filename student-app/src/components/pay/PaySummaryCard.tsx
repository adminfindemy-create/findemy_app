    ...
    <>
      ...
      <View style={[styles.card, { backgroundColor: theme.color.paperWarm, borderColor: theme.color.hairline }]}>
        <View style={styles.line}>
          <Text style={[styles.k, { color: theme.color.inkSoft, fontFamily: theme.font.sansSemibold }]}>{orderLabel}</Text>
          <Text style={[styles.v, { color: theme.color.ink, fontFamily: theme.font.sansSemibold }]}>
            {base != null ? formatRupees(base) : "—"}
          </Text>
        </View>
        <View style={styles.line}>
          <Text style={[styles.k, { color: theme.color.inkSoft, fontFamily: theme.font.sansSemibold }]}>Fees &amp; charges</Text>
          <Text style={[styles.v, { color: theme.color.ink, fontFamily: theme.font.sansSemibold }]}>{formatRupees(fee)}</Text>
        </View>
        <View style={[styles.divider, { backgroundColor: theme.color.hairline }]} />
        <View style={styles.line}>
          <Text style={{ fontFamily: theme.font.sansBold, fontSize: 15, color: theme.color.ink }}>Grand total</Text>
          {loading ? (
