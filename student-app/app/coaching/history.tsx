...
import { IconCal, IconClock, Summary, SummaryRow, useTheme } from '@findemy/ui';
            ...
            value={
              booking.checked_out_at
                ? `${END_REASON_LABEL[endReason ?? ''] ?? 'Completed'}${
                    refundStatus
                      ? ` · ${refundAmountPaise != null ? formatRupees(refundAmountPaise) : '—'} refund`
                      : ''
                  }`
                : '—'
            }
        ...
        >
          {sorted.map((booking) => (
            <HistoryRow
              key={booking.id}
              booking={booking}
              onPress={() => router.push(`/coaching/${booking.id}` as any)}
            />
          ))}
        </ScrollView>
      )}
