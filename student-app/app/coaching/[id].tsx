    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      ...
      >
        {booking.status === 'requested' ? (
          ...
          <StatusBanner tone="rose" icon={<IconX size={16} color={theme.color.rose} />}>
            {booking.rejected_reason
              ? `Declined — ${booking.rejected_reason}`
        ...
        <Summary>
          ...
          {booking.academy_name ? (
            <SummaryRow label="Academy" value={booking.academy_name} />
          ) : null}
          <SummaryRow
            label="Amount"
            value={
              booking.amount_paise != null
                ? formatRupees(booking.amount_paise)
                : 'Quoted on acceptance'
            }
            last
          />
        </Summary>
        ...
        {isCompleted ? (
          <Summary style={{ marginTop: 14 }}>
            ...
            <SummaryRow
              ...
              value={
                refundStatus
                  ? `${refundAmountPaise != null ? formatRupees(refundAmountPaise) : '—'} · ${refundStatus}`
                  : '—'
              }
              last
            />
          ...
          >
            ...
            {coachPhone ? (
              <View style={{ marginTop: 12 }}>
                <Button
                  variant="soft"
                  icon={<IconWa size={16} color={theme.color.persimmonDeep} />}
