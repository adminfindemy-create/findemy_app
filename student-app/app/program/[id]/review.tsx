...
import { DISCOUNT_BPS_CAP, PLAN_MONTHS, type Plan } from '@findemy/types';
import {
  ActionBar,
  BlockPrintCover,
  Button,
...
export default function EnrollSelectScreen() {
  ...
  const handlePay = async () => {
    ...
    setPaying(true);
    try {
      const result = await enrollBatch.mutateAsync({
        batchId: batch.id,
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 130 }}>
        ...
        <View style={[styles.policy, { backgroundColor: theme.color.paperWarm }]}>
          ...
          >
            ...
            Pause, transfer or discontinue anytime from Your Classes. Cancel before renewal to avoid
            charges.
          </Text>
        </View>
      </ScrollView>
      ...
      <ActionBar
        priceLabel="TOTAL"
        priceValue={inr(selected?.amount ?? 0)}
