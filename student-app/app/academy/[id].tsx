...
import { BlockPrintCover, IconArrowR, IconChevL, IconMappin, useTheme } from '@findemy/ui';
...
function Stat({
  ...
  star,
  onPress,
}: { value: string; label: string; star?: boolean; onPress?: () => void }) {
  ...
  const inner = (
    <>
      ...
      <Text style={[styles.statLbl, { fontFamily: theme.font.sansBold, color: theme.color.mist }]}>
        {label}
      </Text>
    </>
  );
...
function BlockLabel({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Text
      style={[styles.blockLabel, { fontFamily: theme.font.sansBold, color: theme.color.whisper }]}
    >
      {children}
    </Text>
  );
...
export default function AcademyDetailScreen() {
  ...
  const scrollToReviews = () => {
    ...
    scrollRef.current?.scrollTo({
      y: Math.max(0, bodyY.current + reviewsY.current - 12),
      animated: true,
    });
  };
      ...
      >
        ...
        <View style={{ position: 'relative', height: 300 }}>
          {academy?.images?.length > 0 ? (
            ...
            <BlockPrintCover
              category={academy?.category ?? 'music'}
              variant={2}
              letter={academy?.name?.[0]}
              height={300}
            />
          )}
          ...
          <SafeAreaView edges={['top']} style={styles.topBar} pointerEvents="box-none">
            <View style={styles.topBarInner}>
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <CircleBtn
                  label="Share"
                  onPress={() => Share.share({ message: `Check out ${academy?.name} on Findemy!` })}
                >
                  <Text style={{ color: '#fff', fontSize: 15 }}>↑</Text>
                </CircleBtn>
                <CircleBtn
                  label={isSaved ? 'Remove from wishlist' : 'Save to wishlist'}
                  onPress={() => toggleSave.mutate(id)}
                >
                  <Text style={{ color: isSaved ? theme.color.persimmon : '#fff', fontSize: 16 }}>
                    {isSaved ? '♥' : '♡'}
                  </Text>
                </CircleBtn>
              </View>
        ...
        >
          <Text
            style={[styles.cat, { fontFamily: theme.font.sansBold, color: theme.color.persimmon }]}
          >
          ...
          >
            {academy?.name}
            {academy?.verified ? (
              <Text style={{ fontSize: 15, color: theme.color.jade }}> ✓</Text>
            ) : null}
          </Text>
          {academy?.address ? (
          ...
          <View style={{ marginTop: 14 }}>
            {tab === 'programs' ? (
              ...
              inlineWorkshops.map((w) => (
                <WorkshopRowCard
                  key={w.id}
                  workshop={w}
                  onPress={() => router.push(`/workshop/${w.id}`)}
                />
              ))
            )}
          </View>
          ...
          {totalOfferings > 2 ? (
            ...
            >
              <Text
                style={{ fontFamily: theme.font.sansBold, fontSize: 13, color: theme.color.ink }}
              >
              ...
              <>
                  ...
                  return (
                    ...
                    >
                      <View style={styles.reviewRow}>
                        <View
                          style={[styles.reviewPic, { backgroundColor: theme.color.persimmon }]}
                        >
                          <Text
                            style={{
                              color: '#fff',
                        ...
                        <Text
                          style={{ color: theme.color.persimmon, fontSize: 12, letterSpacing: 1.5 }}
                        >
                          {'★'.repeat(full)}
                          <Text style={{ color: theme.color.whisper }}>
                ...
                {!reviewsExpanded && (reviews.length > 5 || reviewsHasNextPage) ? (
                  <Pressable
                    ...
                    style={[
                      styles.seeAll,
                      { borderColor: theme.color.hairline, ...theme.shadow.sm, marginTop: 12 },
                    ]}
                  >
                    <Text
                      style={{
          ...
          {coaches.length > 0 ? (
            <View style={{ marginTop: 24 }}>
              ...
              {coaches.map((coach: any) => (
                <View
                  key={coach.id}
                  style={[styles.coach, { borderBottomColor: theme.color.hairline }]}
                >
                  <View style={[styles.coachPic, { backgroundColor: theme.color.persimmon }]}>
                    <Text
                      style={{ color: '#fff', fontFamily: theme.font.serifItalic, fontSize: 16 }}
                    >
