...
import { Button, IconChevL, IconChevR, useTheme } from '@findemy/ui';
import {
  addMonths,
  eachDayOfInterval,
  endOfMonth,
...
export default function BookingSlotScreen() {
  ...
  const { batch_id, reschedule_booking_id } = useLocalSearchParams<{
    batch_id: string;
    ...
    if (reschedule_booking_id) {
      try {
        await rescheduleBooking.mutateAsync({
          id: reschedule_booking_id,
          new_trial_at: selectedSlot,
        });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      ...
      >
        ...
        <View style={[styles.cal, { borderColor: theme.color.hairline, ...theme.shadow.sm }]}>
          <View style={styles.calHead}>
            <Pressable
              ...
              style={[styles.calNav, { borderColor: theme.color.hairline }]}
              accessibilityLabel="Previous month"
            >
              <IconChevL size={16} color={theme.color.ink} />
            </Pressable>
            <Text style={{ fontFamily: theme.font.sansBold, fontSize: 16, color: theme.color.ink }}>
                ...
                >
                  <View
                    style={[
                      ...
                      isToday &&
                        !isSelected && { borderWidth: 1.5, borderColor: theme.color.persimmon },
                    ]}
                  >
                    <Text
                      style={{
        ...
        <Text
          style={[styles.stepLabel, { fontFamily: theme.font.sansBold, color: theme.color.mist }]}
        >
