...
import { BlockPrintCover, IconClock, IconMappin, useTheme } from '@findemy/ui';
...
export default function BookingPayScreen() {
  ...
  const expiresAt = bookingDetail?.reservation_expires_at
    ? new Date(bookingDetail.reservation_expires_at).getTime()
    : null;
      ...
      Alert.alert('Dev mode', 'Razorpay keys not configured. Simulate payment?', [
        {
          ...
          onPress: async () => {
            try {
              await fetch(
                `${process.env.EXPO_PUBLIC_API_URL ?? 'http://localhost:8080'}/payments/webhook`,
                {
                  method: 'POST',
  ...
  const academyAddress = isOnline
    ? 'Online · Findemy room'
    : (bookingDetail?.academy?.address ?? '');
  ...
  const breakdown = order.data?.breakdown as
    | { base_paise: number; total_paise: number }
    | undefined;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      ...
      {mmss ? (
        ...
        >
          <Text
            style={{ fontFamily: theme.font.sansSemibold, fontSize: 13, color: theme.color.mist }}
          >
            Complete your booking in{' '}
            <Text
              style={{
                fontFamily: theme.font.sansBold,
                color: remainingMs && remainingMs < 60000 ? theme.color.rose : theme.color.jade,
              }}
            >
              {mmss}
            </Text>{' '}
      ...
      >
        ...
        <View style={styles.itemRow}>
          {images?.length ? (
            ...
            <BlockPrintCover
              ...
              height={62}
              hideLetter
              style={{ width: 62, height: 62, borderRadius: 14, overflow: 'hidden' }}
            />
          )}
        ...
        >
          ...
          <View style={styles.lineRow}>
            ...
            <Text style={{ fontFamily: theme.font.sansBold, fontSize: 14, color: theme.color.ink }}>
              {breakdown
                ? breakdown.base_paise > 0
                  ? formatRupees(breakdown.base_paise)
                  : 'Free'
                : '—'}
            </Text>
          </View>
          ...
          <View style={styles.dtRow}>
            ...
            >
              {isOnline
                ? 'Join the live class from the app at class time'
                : 'Show your attendance code at the class'}
            </Text>
          </View>
        </View>
        ...
        {/* Payment summary */}
        <PaySummaryCard
          basePaise={breakdown?.base_paise}
          totalPaise={breakdown?.total_paise}
      ...
      <PayFooter
        totalPaise={breakdown?.total_paise}
        onPay={onPay}
        loading={order.isLoading || paying}
        disabled={!order.data}
      />
    </SafeAreaView>
  );
