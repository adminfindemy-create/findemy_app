...
function PaymentRow({
	payment,
	onPress,
}: { payment: PaymentItem; onPress: () => void }) {
...
function ReceiptSheet({
	paymentId,
	onClose,
}: { paymentId: string | null; onClose: () => void }) {
	const theme = useTheme();
	...
	return (
		<BottomSheet visible={!!paymentId} onClose={onClose}>
			...
			{receiptQ.isLoading ? (
				...
				<ErrorState
					code={(receiptQ.error as any)?.code}
					onRetry={receiptQ.refetch}
				/>
			) : receipt ? (
				<>
					<Text
					...
					<Summary style={{ marginTop: 12 }}>
						...
						<SummaryRow
							label="Paid on"
							value={format(new Date(receipt.paid_at), "d MMMM yyyy")}
						/>
						<SummaryRow
							label="Amount"
	...
	return (
		...
		>
			...
			{paymentsQ.isLoading ? (
				...
				<ErrorState
					code={(paymentsQ.error as any)?.code}
