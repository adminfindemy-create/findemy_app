...
function MissedClassRow({ session }: { session: MissedSession }) {
	...
	const dateLabel = (() => {
		...
		return isNaN(parsed.getTime()) ? session.session_date : format(parsed, "EEE, d MMM yyyy");
	})();
		...
		>
			<View style={styles.rowHead}>
				...
				<View style={[styles.badge, { backgroundColor: theme.color.roseSoft }]}>
					<Text style={[styles.badgeText, { fontFamily: theme.font.sansBold, color: theme.color.rose }]}>
						Missed
					</Text>
				</View>
			</View>
			...
			{session.recording_url ? (
				...
				>
					<Text style={[styles.recordingText, { fontFamily: theme.font.sansBold, color: theme.color.persimmon }]}>
						Watch recording ↗
					</Text>
				</Pressable>
			) : null}
