...
const PACKAGE_CONFIG: Record<string, { label: string; months: number; badge?: string; badgeTone?: string }> = {
  monthly:   { label: "Monthly",   months: 1  },
  quarterly: { label: "Quarterly", months: 3,  badge: "Popular",   badgeTone: "jade" },
  annual:    { label: "Annual",    months: 12, badge: "Best value", badgeTone: "persimmon" },
};

const DISCONTINUE_REASONS = [
  { key: "schedule_issue",     label: "Schedule issue",    desc: "The class timings don't work for me anymore" },
  { key: "financial",          label: "Financial reasons", desc: "Budget constraints at the moment" },
  { key: "moving_city",        label: "Moving city",       desc: "Relocating soon" },
  { key: "not_satisfied",      label: "Not satisfied",     desc: "The class didn't meet my expectations" },
  { key: "switching_academy",  label: "Switching academy", desc: "Found a better fit elsewhere" },
...
function BillRow({ label, value, theme, last }: { label: string; value: string; theme: any; last?: boolean }) {
  return (
    <View
      style={{
        flexDirection: "row",
    ...
    >
      <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 13, color: theme.color.mist }}>{label}</Text>
      <Text
        style={{ fontFamily: theme.font.sansBold, fontSize: 13.5, color: theme.color.ink, flex: 1, textAlign: "right" }}
        numberOfLines={1}
      >
        {value}
      </Text>
...
function RenewSheet({ enrollment, onClose }: { enrollment: any; onClose: () => void }) {
  const theme = useTheme();
      ...
      >
        <View
          style={[
            renewStyles.check,
            {
              borderColor: makeDefault ? theme.color.persimmon : theme.color.hairline,
              backgroundColor: makeDefault ? theme.color.persimmon : "transparent",
            },
          ]}
        >
          {makeDefault ? <Text style={{ color: theme.color.ivory, fontSize: 11 }}>✓</Text> : null}
        </View>
        <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 12.5, color: theme.color.inkSoft }}>
...
function DiscontinueSheet({ enrollment, onClose }: { enrollment: any; onClose: () => void }) {
  const theme = useTheme();
  ...
  const handleConfirm = async () => {
    try {
      await discontinueMut.mutateAsync({ id: enrollment.id, immediate: false, reason });
      onClose();
      Alert.alert(
    ...
    return (
      <View style={{ paddingHorizontal: 24, paddingBottom: 40 }}>
        ...
        <View
          style={[
            discStyles.infoBox,
            { backgroundColor: theme.color.marigoldSoft, borderColor: theme.color.marigold },
          ]}
        >
          <Text style={{ fontFamily: theme.font.sans, fontSize: 13, color: theme.color.inkSoft, lineHeight: 19 }}>
            Your access to{" "}
            <Text style={{ fontFamily: theme.font.sansBold }}>{enrollment.batch_title}</Text> will continue
        ...
        >
          After that, your enrollment will be marked as ended. You can re-enroll anytime.
        </Text>
        <View style={{ gap: 10 }}>
          <Button variant="rose" block onPress={handleConfirm} loading={discontinueMut.isPending}>
            Confirm discontinuation
          </Button>
  ...
  return (
    <View style={{ paddingHorizontal: 24, paddingBottom: 40 }}>
      ...
      <View style={{ marginTop: 8 }}>
        <Button variant="rose" block disabled={!reason} onPress={() => setStep("confirm")}>
          Continue
        </Button>
      </View>
    </View>
...
function PauseSheet({ enrollment, onClose }: { enrollment: any; onClose: () => void }) {
  const theme = useTheme();
  ...
  return (
    <View style={{ paddingHorizontal: 24, paddingBottom: 40 }}>
      ...
      >
        Your spot is held and your paid period is extended by the pause duration when you resume.
      </Text>
            ...
            <Pressable
              ...
              style={[
                pauseStyles.durationPill,
                {
                  ...
                  borderColor: isSelected ? theme.color.ink : theme.color.hairline,
                },
              ]}
            >
              <Text
...
function TransferSheet({ enrollment, onClose }: { enrollment: any; onClose: () => void }) {
  const theme = useTheme();
  const [reason, setReason] = useState("");
  ...
  return (
    <View style={{ paddingHorizontal: 24, paddingBottom: 40 }}>
      ...
      />
      <View style={{ marginTop: 18 }}>
        <Button block onPress={handleTransfer} loading={transferMut.isPending} disabled={!targetBatchId}>
          Confirm transfer
        </Button>
      </View>
    </View>
...
function JoinClassButton({ enrollment, theme }: { enrollment: any; theme: any }) {
  const router = useRouter();
  ...
  if (enrollment.mode !== "online") {
    return (
      <Pressable
        ...
        style={[joinStyles.btn, { backgroundColor: theme.color.jade, flexDirection: "row", justifyContent: "center", gap: 9 }]}
      >
        <IconQr size={18} color={theme.color.ivory} />
        <Text style={[joinStyles.btnText, { color: theme.color.ivory }]}>Scan to check in</Text>
      </Pressable>
    );
  }
  ...
  return (
    ...
    >
      <Text style={[joinStyles.btnText, { color: theme.color.ivory }]}>Join Class →</Text>
    </Pressable>
  );
}

const joinStyles = StyleSheet.create({
  btn: { borderRadius: 14, paddingVertical: 14, alignItems: "center", marginHorizontal: 24, marginBottom: 16 },
  btnText: { fontSize: 15, fontWeight: "700", letterSpacing: 0.2 },
  pill: { alignSelf: "flex-start", marginHorizontal: 24, marginBottom: 16, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999 },
...
export default function EnrollmentDetailScreen() {
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={["top"]}>
        ...
        <View style={{ paddingHorizontal: 24, paddingTop: 12, gap: 12 }}>
          ...
          <SkeletonLoader height={80} borderRadius={14} style={{ marginTop: 8 }} />
  ...
  const isDiscontinuePending = !!(detail?.discontinue_requested_at ?? enrollment.discontinue_requested_at);
  const isInactive = enrollment.status !== "active";
  // Batch being discontinued — renewals are blocked; existing term serves out.
  const isBatchClosing = detail?.batch_status === "closing" || detail?.batch_status === "ended";
  ...
  const scheduleLine = [isOnline ? "Online" : null, batchDayPattern(timings), timings[0] ? formatTimeRange(timings[0]) : null]
    .filter(Boolean)
    .join(" · ");
  ...
  const statusConfig = (() => {
    if (isPaused) return { label: "● Paused", bg: theme.color.marigoldSoft, fg: theme.color.marigold };
    if (isDiscontinuePending) return { label: "● Ending", bg: theme.color.roseSoft, fg: theme.color.rose };
    if (enrollment.status === "active") return { label: "● Active", bg: theme.color.jadeSoft, fg: theme.color.jade };
  ...
  const handleCancelDiscontinue = async () => {
    try {
      ...
      Alert.alert("Discontinuation cancelled", "Your enrollment will continue as normal.");
    } catch { /* onError handles */ }
  };
  ...
  const handleResume = async () => {
    ...
    try {
      await resumeMut.mutateAsync({ enrollmentId: enrollment.id, pauseId: activePauseId });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={["top"]}>
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        {/* ── Summary card ── */}
        <View style={[styles.sumCard, { borderColor: theme.color.hairline, ...theme.shadow.sm }]}>
          <BlockPrintCover category={enrollment.category as any} variant={1} height={74} hideLetter style={styles.sumThumb} />
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text style={[styles.sumTtl, { fontFamily: theme.font.sansBold, color: theme.color.ink }]} numberOfLines={1}>
              {enrollment.batch_title}
            </Text>
            <Text style={[styles.sumSub, { fontFamily: theme.font.sansMedium, color: theme.color.mist }]} numberOfLines={1}>
              {[enrollment.academy_name, enrollment.coach_name].filter(Boolean).join(" · ")}
            </Text>
            <View style={styles.sumFoot}>
              <Text style={[styles.sumSchedule, { fontFamily: theme.font.sansMedium, color: theme.color.mist }]} numberOfLines={1}>
                {scheduleLine}
              </Text>
              {fee ? <Text style={[styles.sumFee, { fontFamily: theme.font.sansBold, color: theme.color.ink }]}>{fee}</Text> : null}
            </View>
          </View>
        </View>
        ...
        {isDiscontinuePending ? (
            ...
            right={
              <Pressable onPress={handleCancelDiscontinue} hitSlop={8}>
                {cancelDiscMut.isPending ? (
                  ...
                  <Text style={{ fontFamily: theme.font.sansBold, fontSize: 12.5, color: theme.color.rose }}>Undo</Text>
                )}
              </Pressable>
            }
          >
          ...
          <StatusBanner tone="jade" icon={<IconCheck size={18} color={theme.color.jade} />}>
        ...
        {enrollment.status === "active" && !isOnline ? (
          <View style={{ marginBottom: 4 }}>
            <Button variant="ghost" block onPress={handleDirections}>Get directions</Button>
          </View>
        ) : null}
        ...
        <Summary>
          {enrollment.coach_name ? (
            <SummaryRow icon={<IconUser size={18} color={theme.color.persimmon} />} label="Coach" value={enrollment.coach_name} />
          ) : null}
          <SummaryRow
          ...
          />
          {isOnline ? (
            <SummaryRow icon={<IconMappin size={18} color={theme.color.persimmon} />} label="Mode" value="Online · link active 10 min before" />
          ) : (
            <SummaryRow
              icon={<IconMappin size={18} color={theme.color.persimmon} />}
              label="Venue"
              value={enrollment.academy_address ?? enrollment.academy_name ?? "—"}
            />
          )}
          <SummaryRow icon={<IconClock size={18} color={theme.color.persimmon} />} label="Next class" value={nextClassLabel} last />
        </Summary>
        ...
        <Summary>
          ...
          {planLabel ? <BillRow label="Plan" value={planLabel} theme={theme} /> : null}
          {fee ? <BillRow label="Fee" value={fee} theme={theme} /> : null}
          {nextRenewal ? <BillRow label="Next renewal" value={format(new Date(nextRenewal), "d MMM yyyy")} theme={theme} /> : null}
          <BillRow label="Payment method" value="UPI · Razorpay" theme={theme} />
          <BillRow
            label="Member since"
            value={enrollment.started_at ? format(new Date(enrollment.started_at), "MMM yyyy") : "—"}
            theme={theme}
            last
          />
        </Summary>
        ...
        <Summary>
          <Text style={{ fontFamily: theme.font.serif, fontSize: 22, color: theme.color.ink }}>
            {attendedCount > 0 ? `${attendedCount} ${attendedCount === 1 ? "class" : "classes"}` : "No classes yet"}
          </Text>
          {attendedCount > 0 ? (
            <>
              <View style={styles.dotTrack}>
                ...
                {attendedCount > DOT_COUNT ? (
                  <Text style={[styles.dotOverflow, { fontFamily: theme.font.sans, color: theme.color.mist }]}>
                    +{attendedCount - DOT_COUNT} more
                  </Text>
                ) : null}
              </View>
              <View style={[styles.progressTrack, { backgroundColor: theme.color.hairline }]}>
                <View
                  style={[styles.progressFill, { backgroundColor: catColors.accent, width: `${(filledDots / DOT_COUNT) * 100}%` }]}
                />
              </View>
            </>
          ) : null}
        ...
        <MenuRow
          ...
          disabledReason={isBatchClosing ? "This batch is being discontinued" : "Enrollment ended"}
        ...
        <MenuRow
          ...
          disabledReason={isDiscontinuePending ? "Already ending" : "Enrollment ended"}
      ...
      {/* ── Sub-sheet host ── */}
      <Modal visible={!!subSheet} transparent animationType="slide" onRequestClose={closeSheet}>
        <Pressable style={sheetStyles.backdrop} onPress={closeSheet} />
        <View style={[sheetStyles.sheet, { backgroundColor: theme.color.paper }]}>
          {subSheet === "renew" && <RenewSheet enrollment={enrollment} onClose={closeSheet} />}
          {subSheet === "discontinue" && <DiscontinueSheet enrollment={enrollment} onClose={closeSheet} />}
          {subSheet === "pause" && <PauseSheet enrollment={enrollment} onClose={closeSheet} />}
          {subSheet === "transfer" && <TransferSheet enrollment={enrollment} onClose={closeSheet} />}
        </View>
      </Modal>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  sumFoot: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8, marginTop: 8 },
  ...
  dotTrack: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: 12, marginBottom: 10 },
