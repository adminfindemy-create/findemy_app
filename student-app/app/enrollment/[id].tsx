  ...
  if (step === 'confirm') {
    return (
      <View style={{ paddingHorizontal: 24, paddingBottom: 40 }}>
        ...
        <View style={sheetStyles.titleRow}>
          <Text
            style={[sheetStyles.title, { fontFamily: theme.font.serif, color: theme.color.ink }]}
          >
        ...
        >
          ...
          >
            Your access to{' '}
            <Text style={{ fontFamily: theme.font.sansBold }}>{enrollment.batch_title}</Text> will
          ...
          {siblings.map((siblingBatch: any) => {
            ...
            const seats =
              siblingBatch.capacity != null
                ? Number(siblingBatch.capacity) - Number(siblingBatch.enrolled_count ?? 0)
                : null;
            const parts = [
              time,
              seats != null ? `${Math.max(seats, 0)} seat${seats === 1 ? '' : 's'} left` : '',
            ].filter(Boolean);
            return (
              <OptionRow
                key={siblingBatch.id}
...
export default function EnrollmentDetailScreen() {
  ...
  const catColors =
    theme.category[enrollment.category as keyof typeof theme.category] ?? theme.category.music;
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.sumCard, { borderColor: theme.color.hairline, ...theme.shadow.sm }]}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text
              style={[styles.sumTtl, { fontFamily: theme.font.sansBold, color: theme.color.ink }]}
              numberOfLines={1}
            >
        ...
        <Summary>
          ...
          <BillRow
            label="Member since"
            value={
              enrollment.started_at ? format(new Date(enrollment.started_at), 'MMM yyyy') : '—'
            }
            theme={theme}
            last
          />
                ...
                {Array.from({ length: DOT_COUNT }).map((_, index) => (
                  <View
                    ...
                    style={[
                      ...
                      {
                        backgroundColor:
                          index < filledDots ? catColors.accent : theme.color.hairline,
                      },
                    ]}
                  />
                ))}
        ...
        <MenuRow
          ...
          onPress={() => router.push(`/resources/${enrollment.batch_id}` as any)}
        />
