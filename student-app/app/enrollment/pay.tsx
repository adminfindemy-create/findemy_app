...
import { BlockPrintCover, IconCal, IconShield, useTheme } from '@findemy/ui';
      ...
      Alert.alert('Dev mode', 'Razorpay keys not configured. Simulate payment?', [
        {
          ...
          onPress: async () => {
            ...
            router.replace(
              `/enrollment/confirmation?enrollment_id=${enrollment_id}&enrollment_period_id=${enrollment_period_id}&batch_title=${encodeURIComponent(batch_title ?? '')}`
            );
          },
        },
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      ...
      >
        ...
        <View style={styles.itemRow}>
          <BlockPrintCover
            category={(category as any) ?? 'music'}
            variant={1}
            height={62}
            hideLetter
            style={styles.thumb}
      ...
      <PayFooter
        totalPaise={amountNum}
        onPay={onPay}
        loading={paying}
        ctaLabel={isRenewal ? 'Pay & renew' : 'Pay & enrol'}
      />
    </SafeAreaView>
  );
}
