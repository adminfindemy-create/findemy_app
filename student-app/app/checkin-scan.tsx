...
import { IconCheck, IconX, tokens, useTheme } from '@findemy/ui';
...
export default function CheckinScanScreen() {
  ...
  const { class_name, academy_name } = useLocalSearchParams<{
    class_name?: string;
    academy_name?: string;
  }>();
  const [done, setDone] = useState<string | null>(null);

  const _submit = async (token: string) => {
    try {
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.color.ink }]}>
        <View style={styles.center}>
          ...
          <Text style={[styles.heading, { color: '#fff', marginTop: 18 }]}>
            You're marked present
          </Text>
          <Text style={[styles.sub, { marginTop: 6 }]}>{done}</Text>
          <Pressable
            onPress={() => router.back()}
            style={[styles.btn, { backgroundColor: theme.color.jade }]}
          >
        ...
        {class_name || academy_name ? (
          <View style={{ alignItems: 'center', marginTop: 24 }}>
            ...
            <Text style={styles.classSub}>
              {academy_name ? `${academy_name} · ` : ''}today's session
            </Text>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  topTitle: { fontFamily: tokens.font.sansBold, fontSize: 17, color: '#fff' },
  intro: {
  ...
  scanLine: {
    ...
    right: 12,
    height: 2,
    backgroundColor: 'rgba(236,90,43,0.85)',
  },
  className: { fontFamily: tokens.font.sansBold, fontSize: 16, color: '#fff' },
  classSub: {
  ...
  heading: { fontFamily: tokens.font.sansBold, fontSize: 22, textAlign: 'center' },
  sub: {
    fontFamily: tokens.font.sans,
    fontSize: 14,
    textAlign: 'center',
  ...
  btnText: { fontFamily: tokens.font.sansBold, fontSize: 15, color: '#fff' },
  hint: {
    fontFamily: tokens.font.sans,
    fontSize: 12,
