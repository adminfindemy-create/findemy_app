  ...
  return (
    ...
    >
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text
          style={[styles.renewalTitle, { fontFamily: theme.font.sansBold, color: theme.color.ink }]}
          numberOfLines={1}
        >
...
function PendingSessionRow({ session }: { session: UpcomingSession }) {
  ...
  return (
    <View
      style={[styles.pendingRow, { backgroundColor: '#fff', borderColor: theme.color.hairline }]}
    >
      <View style={[styles.pendingDate, { backgroundColor: theme.color.paperWarm }]}>
        <Text
          style={[
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text
          style={[styles.pendingTitle, { fontFamily: theme.font.sansBold, color: theme.color.ink }]}
          numberOfLines={1}
        >
          {session.batch_title}
        </Text>
...
export default function ClassesScreen() {
  ...
  const pendingSessions = (sessionsQ.data?.items ?? []).slice(0, PENDING_SESSIONS_PREVIEW_COUNT);
  ...
  const SectionLabel = ({ children }: { children: string }) => (
    <Text
      style={[styles.sectionLabel, { fontFamily: theme.font.sansBold, color: theme.color.whisper }]}
    >
      {children}
    </Text>
  );
  ...
  const footFor = (classItem: ClassItem) =>
    [classItem.academy_name, coachFirst(classItem.coach_name)].filter(Boolean).join(' · ');
  ...
  const renewalDue = active.filter((classItem) => {
    if (classItem.status === 'paused' || classItem.batch_status === 'closing') return false;
    if (!classItem.current_period_end) return false;
    if (dismissedRenewals.has(classItem.enrollment_id)) return false;
    return (
      differenceInCalendarDays(new Date(classItem.current_period_end), new Date()) <=
      RENEWAL_WINDOW_DAYS
    );
  });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      <View style={styles.head}>
        ...
        <Pressable
          onPress={() => router.push('/missed-classes' as any)}
          accessibilityRole="button"
          accessibilityLabel="View missed classes"
          style={{ marginTop: 10 }}
        >
          <Text
            style={{ fontFamily: theme.font.sansBold, fontSize: 12.5, color: theme.color.mist }}
          >
            Missed classes →
          </Text>
        </Pressable>
      ...
      {isLoading ? (
        ...
        <View style={styles.empty}>
          ...
          >
            Enroll in a class to get started — your enrolled classes will show up here. Trials,
            workshops and events live in Your Bookings.
          </Text>
            ...
            <View style={{ gap: 10, marginBottom: 6 }}>
              {renewalDue.map((classItem) => (
                <RenewalDueCard
                  ...
                  classItem={classItem}
                  onPay={() =>
                    router.push(`/enrollment/${classItem.enrollment_id}?openRenew=1` as any)
                  }
              ...
              {active.map((classItem) => {
                ...
                const resumes =
                  paused && classItem.paused_until
                    ...
                    : undefined;
                const schedule = [online ? 'Online' : undefined, scheduleSummary(classItem.timings)]
                  .filter(Boolean)
                  .join(' · ');
            ...
            <>
              ...
              {past.map((classItem) => (
                <RowCard
                  ...
                  subText={format(
                    new Date(classItem.current_period_end ?? classItem.started_at),
                    'MMM yyyy'
                  )}
                  footLeft={footFor(classItem)}
