...
function eventDate(iso?: string) {
  ...
  return Number.isNaN(date.getTime()) ? '' : format(date, 'd MMM · EEE');
}
...
export default function EventsScreen() {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={['top']}>
      ...
      >
        ...
        <View style={styles.head}>
          <Text
            style={[styles.kicker, { fontFamily: theme.font.sansBold, color: theme.color.mist }]}
          >
            BY FINDEMY
          </Text>
          <Text
          ...
          >
            <Text style={{ fontFamily: theme.font.serifItalic, color: theme.color.inkSoft }}>
              Events &{'\n'}
            </Text>
            ...
            events.isLoading && workshops.isLoading ? (
              ...
              <>
                {spotlight ? (
                  <Spotlight
                    event={spotlight}
                    onPress={() => router.push(`/events/${spotlight.id}`)}
                  />
                ) : null}
                <View style={styles.sectionHead}>
                  <Text
                    style={[styles.st, { fontFamily: theme.font.sansBold, color: theme.color.ink }]}
                  >
                    This month
                  </Text>
                  <Text
  ...
  function Spotlight({ event, onPress }: { event: any; onPress: () => void }) {
    const meta = [eventDate(event.start_at).split(' · ')[0], event.location]
      .filter(Boolean)
      .join(' · ');
    return (
      <Pressable onPress={onPress} style={styles.spotlight}>
        ...
        <View>
          <Text style={styles.spBy}>
            {(event.organizer_name ? `By ${event.organizer_name}` : 'By Findemy').toUpperCase()}
          </Text>
