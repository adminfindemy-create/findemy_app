...
import { useTheme, Button, Input, BlockPrintCover, IconX, IconStar } from "@findemy/ui";
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40 }}>
          <View style={styles.doneHero}>
            ...
            <Text style={{ fontFamily: theme.font.serif, fontSize: 34, color: theme.color.ink, textAlign: "center", marginTop: 18, lineHeight: 38 }}>
              Thanks for the review!
            </Text>
            <Text style={{ fontFamily: theme.font.sans, fontSize: 14, color: theme.color.mist, textAlign: "center", marginTop: 6 }}>
          ...
          <View style={[styles.card, { backgroundColor: "#fff", borderColor: theme.color.hairline, ...theme.shadow.sm }]}>
            <BlockPrintCover category={"music" as any} variant={1} height={66} hideLetter style={styles.thumb} />
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text style={{ fontFamily: theme.font.sansBold, fontSize: 16, color: theme.color.ink }} numberOfLines={1}>{title}</Text>
              {academy ? <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 12.5, color: theme.color.mist, marginTop: 3 }}>{academy}</Text> : null}
            </View>
          </View>

          <View style={{ marginTop: 20, gap: 10 }}>
            <Button block onPress={goEnroll}>Enrol now</Button>
            <Button variant="ghost" block onPress={() => router.replace("/(tabs)")}>Maybe later</Button>
          </View>
        </ScrollView>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={["top"]}>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40 }}>
        ...
        <Text style={{ fontFamily: theme.font.serif, fontSize: 34, color: theme.color.ink, marginTop: 6, marginBottom: 4 }}>
          How was it?
        </Text>

        {/* Trial card */}
        <View style={[styles.card, { backgroundColor: "#fff", borderColor: theme.color.hairline, ...theme.shadow.sm, marginTop: 12 }]}>
          <BlockPrintCover category={"music" as any} variant={1} height={66} hideLetter style={styles.thumb} />
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text style={{ fontFamily: theme.font.sansBold, fontSize: 16, color: theme.color.ink }} numberOfLines={1}>{title}</Text>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 4 }}>
              <View style={[styles.badge, { backgroundColor: theme.color.jadeSoft }]}>
                <Text style={{ fontFamily: theme.font.sansBold, fontSize: 10, color: theme.color.jade, letterSpacing: 0.4 }}>Attended</Text>
              </View>
              {academy ? <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 12.5, color: theme.color.mist }} numberOfLines={1}>{academy}</Text> : null}
            </View>
          </View>
        </View>
        ...
        <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 13, color: theme.color.mist, textAlign: "center", marginTop: 4 }}>
        ...
        <View style={{ marginTop: 18 }}>
          <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 13, color: theme.color.mist, marginBottom: 8 }}>Add a note (optional)</Text>
          <Input value={comment} onChangeText={setComment} placeholder="What made it special — or not?" multiline />
        </View>

        <View style={{ marginTop: 22 }}>
          <Button variant="dark" block onPress={() => submit.mutate()} loading={submit.isPending} disabled={!canSubmit}>
            Submit review
          </Button>
          <Pressable onPress={() => router.replace("/(tabs)")} style={{ alignItems: "center", paddingVertical: 14 }}>
            <Text style={{ fontFamily: theme.font.sansBold, fontSize: 13, color: theme.color.mist }}>Skip for now</Text>
          </Pressable>
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: "row", alignItems: "center", gap: 13, borderWidth: 1, borderRadius: 20, padding: 12 },
