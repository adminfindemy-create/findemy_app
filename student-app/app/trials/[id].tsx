...
import { View, Text, ScrollView, Alert, StyleSheet, ActivityIndicator, Platform, ToastAndroid, Linking } from "react-native";
...
export default function TrialDetailScreen() {
  ...
  const trialAmountPaise =
    trial.payment_status === "captured" ? trial.payment_amount_paise ?? trial.trial_fee_paise ?? 0 : trial.trial_fee_paise ?? 0;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }} edges={["top"]}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }}>
        ...
        <View style={[styles.card, { backgroundColor: "#fff", borderColor: theme.color.hairline, ...theme.shadow.sm }]}>
          <BlockPrintCover category={category as any} variant={1} height={66} hideLetter style={styles.thumb} />
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            <View style={styles.subRow}>
              ...
              <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 12.5, color: theme.color.mist }} numberOfLines={1}>
                {trialDate ? format(trialDate, "EEE d MMM · h:mm a") : ""}
              </Text>
            </View>
            <View style={styles.foot}>
              <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 12.5, color: theme.color.mist, flex: 1 }} numberOfLines={1}>
                {trial.academy_name ?? ""}
              </Text>
              {trialAmountPaise > 0 ? (
                <Text style={{ fontFamily: theme.font.sansBold, fontSize: 14, color: theme.color.ink }}>{inr(trialAmountPaise)}</Text>
              ) : null}
            </View>
          </View>
        </View>
        ...
        <Summary>
          ...
          />
          {trial.coach_name ? (
            <SummaryRow icon={<IconUser size={18} color={theme.color.persimmon} />} label="Coach" value={trial.coach_name} last={online} />
          ) : null}
          {online ? (
            <SummaryRow icon={<IconMappin size={18} color={theme.color.persimmon} />} label="Mode" value="Online · link active 10 min before" last />
          ) : (
            <>
              <SummaryRow icon={<IconMappin size={18} color={theme.color.persimmon} />} label="Venue" value={trial.academy_name ?? "—"} />
              <SummaryRow icon={<IconMappin size={18} color={theme.color.persimmon} />} label="Address" value={trial.academy_address ?? "—"} last />
            </>
          )}
        </Summary>
        ...
        {isUpcoming ? (
          online ? (
            <View style={{ marginTop: 14 }}>
              <Button variant="jade" block onPress={() => Alert.alert("Online session", "Your join link becomes active 10 minutes before the session.")}>
                Join now
              </Button>
            </View>
          ) : (
        ...
        <View style={[styles.ordCard, { backgroundColor: "#fff", borderColor: theme.color.hairline, ...theme.shadow.sm }]}>
          <OrdRow label="Item" value={trial.batch_title ?? "Trial"} theme={theme} />
          <OrdRow label="Amount paid" value={trialAmountPaise > 0 ? inr(trialAmountPaise) : "Free"} theme={theme} />
          {trial.payment_status === "captured" ? <OrdRow label="Payment method" value="UPI · Razorpay" theme={theme} /> : null}
          <OrdRow label="Order ID" value={String(bookingId)} theme={theme} />
          {bookedOn ? <OrdRow label="Booked on" value={format(new Date(bookedOn), "d MMM yyyy")} theme={theme} last /> : null}
        </View>
        ...
        {trial.refund_status ? (
          <Text
            style={{
              ...
              marginTop: 10,
              color:
                trial.refund_status === "processed" ? theme.color.jade : trial.refund_status === "failed" ? theme.color.rose : theme.color.marigold,
            }}
          >
            {trial.refund_status === "processed" ? "Refund processed" : trial.refund_status === "failed" ? "Refund failed — contact support" : "Refund processing"}
          </Text>
        ) : null}
        ...
        {isAttended ? (
          enrollStatus.data?.enrolled ? (
            <View style={[styles.enrolled, { backgroundColor: theme.color.jadeSoft }]}>
              <Text style={{ fontFamily: theme.font.sansBold, fontSize: 14, color: theme.color.jade }}>✓ Enrolled in this batch</Text>
            </View>
          ) : (
            <View style={{ marginTop: 16 }}>
              <Button block onPress={handleJoinBatch} loading={enrollBatch.isPending}>Join this batch</Button>
            </View>
          )
        ) : null}
        ...
        {isUpcoming ? (
          <>
            ...
            <MenuRow title="Reschedule" sub="Move to another date or time" onPress={handleReschedule} />
            <MenuRow
              title="Cancel trial"
...
function OrdRow({ label, value, theme, last }: { label: string; value: string; theme: any; last?: boolean }) {
  return (
    <View style={[styles.ordRow, { borderBottomColor: theme.color.hairline, borderBottomWidth: last ? 0 : 1 }]}>
      <Text style={{ fontFamily: theme.font.sansMedium, fontSize: 13, color: theme.color.mist }}>{label}</Text>
      <Text style={{ fontFamily: theme.font.sansBold, fontSize: 13.5, color: theme.color.ink, flex: 1, textAlign: "right" }} numberOfLines={1}>
        {value}
      </Text>
    </View>
  );
...
const styles = StyleSheet.create({
  card: { flexDirection: "row", alignItems: "center", gap: 13, borderWidth: 1, borderRadius: 20, padding: 12, marginTop: 4 },
  ...
  ordRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12, paddingVertical: 11 },
