...
import { IconClock, IconMappin, useTheme } from '@findemy/ui';
          ...
          onPress: async () => {
            ...
            try {
                  ...
                  payload: {
                    payment: {
                      entity: {
                        ...
                        notes: { workshop_registration_id: registration_id },
                      },
                    },
                  },
                }),
              ...
              if (!response.ok) {
                Alert.alert(
                  'Webhook failed',
                  `API returned ${response.status}. Cannot confirm registration.`
                );
                return;
              }
            } catch (error: any) {
              Alert.alert(
                'Cannot reach API',
                `Webhook URL ${apiUrl} unreachable from device. ${error?.message ?? ''}`
              );
              return;
            }
            router.replace(
              `/workshop/confirmation?registration_id=${registration_id}&workshop_id=${workshop_id}&title=${encodeURIComponent(title ?? '')}`
            );
          },
        },
      ...
      >
        ...
        <View style={styles.itemRow}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {online ? 'Online workshop' : 'Workshop'}
              {workshop?.academy_name ? ` · ${workshop.academy_name}` : ''}
            </Text>
          </View>
        </View>
