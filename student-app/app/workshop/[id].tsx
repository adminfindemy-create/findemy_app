...
export default function WorkshopDetailScreen() {
  ...
  const dateValid = startAt && !Number.isNaN(startAt.getTime());
  const canCancelRegistration = isRegistered && !!regStatus?.registration_id;

  const cancelTarget: CancelSheetTarget | null =
    startAt && workshop
      ? {
  ...
  const handleCancelRegistration = async ({
    ...
    try {
      ...
      if (response.refund_initiated) {
        showCancelToast(
          `Refund of ${formatRupees(response.refund_amount_paise)} initiated — arrives in 5–7 business days.`
        );
      } else {
  ...
  const price =
    workshop.price_paise === 0
      ? 'Free'
      : `₹${Math.round(workshop.price_paise / 100).toLocaleString('en-IN')}`;
  ...
  const isJoinable =
    ...
    (() => {
      ...
      const now = Date.now();
      return (
        now >= startAt?.getTime() - 10 * 60 * 1000 &&
        now <= startAt?.getTime() + durationMin * 60 * 1000
      );
    })();
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 130 }}>
        ...
        <View style={styles.hero}>
          ...
          <View style={styles.heroBar}>
            <Pressable
              onPress={() => router.back()}
              style={[styles.roundBtn, { backgroundColor: 'rgba(255,255,255,0.92)' }]}
            >
              <IconChevL size={20} color={theme.color.ink} />
            </Pressable>
            <Pressable
              onPress={() => setSaved((prevSaved) => !prevSaved)}
              style={[styles.roundBtn, { backgroundColor: 'rgba(255,255,255,0.92)' }]}
            >
        ...
        <View style={{ paddingHorizontal: 20, marginTop: 16 }}>
          ...
          <Summary style={{ marginTop: 16 }}>
            <SummaryRow
              ...
              value={
                dateValid
                  ? `${format(startAt!, 'EEE, d MMM · h:mm a')}${workshop.duration_min ? ` · ${workshop.duration_min} min` : ''}`
                  : '—'
              }
            />
            {workshop.location ? (
        ...
        {regStatusQ.isLoading ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            {canCancelRegistration && (
              ...
              >
                <Text
                  style={{ fontFamily: theme.font.sansBold, fontSize: 13, color: theme.color.rose }}
                >
                  Cancel registration
                </Text>
              </Pressable>
