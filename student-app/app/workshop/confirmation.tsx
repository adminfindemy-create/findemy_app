...
import { useTheme, Button, Summary, SummaryRow, IconCheck, IconCal } from "@findemy/ui";
...
export default function WorkshopConfirmationScreen() {
  ...
  const { workshop_id, title } =
    useLocalSearchParams<{ registration_id: string; workshop_id: string; title: string }>();
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.color.paper }}>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40 }}>
        ...
        <View style={styles.hero}>
          ...
          >
            {"You're "}
            <Text style={{ fontFamily: theme.font.serifItalic, color: theme.color.jade }}>registered</Text>
            {"!"}
          </Text>
          <Text
            style={{
    ...
    return (
      <View style={[styles.thumbRow, { borderBottomColor: theme.color.hairline, borderBottomWidth: last ? 0 : 1 }]}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text style={{ fontFamily: theme.font.sansBold, fontSize: 11, letterSpacing: 1.1, color: theme.color.whisper }}>
            {label.toUpperCase()}
          </Text>
          <Text style={{ fontFamily: theme.font.sansBold, fontSize: 14.5, color: theme.color.ink, marginTop: 1 }} numberOfLines={2}>
            {value}
          </Text>
        </View>
      </View>
