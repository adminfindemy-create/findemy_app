...
export default function BatchNotesScreen() {
	...
	const [sheetNote, setSheetNote] = useState<Note | null | undefined>(
		undefined,
	);

	const enrollment = useMemo(
		() =>
			(enrollments.data?.items ?? []).find(
		...
		>
			...
			>
				...
				<Summary>
					...
					{categoryValue ? (
						<SummaryRow
							label="Subject"
							value={
								String(categoryValue).charAt(0).toUpperCase() +
								String(categoryValue).slice(1)
							}
							last
						/>
					) : null}
				...
				{notesQuery.isLoading ? (
					...
					<ErrorState
						code={(notesQuery.error as any)?.code}
			...
			>
				<Pressable
					...
					onPress={() => setSheetNote(undefined)}
				/>
				<View
				...
				>
					{sheetNote !== undefined ? (
						<NoteSheet
							batchId={batchId}
							note={sheetNote}
							onClose={() => setSheetNote(undefined)}
						/>
					) : null}
				</View>
			</Modal>
